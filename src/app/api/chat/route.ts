...
// (Unused, kept for reference)
// const formatCareerResponse = (careerData: Record<string, unknown>) => {
//   return `Based on your interest, here's information about becoming a ${careerData['title']}:

// **Description:** ${careerData['description']}

// **Education Required:** 
// ${(careerData['education'] as string[]).map((item: string) => `- ${item}`).join('\n')}

// **Key Skills:**
// ${(careerData['skills'] as string[]).map((item: string) => `- ${item}`).join('\n')}
...
// **Entry Paths:**
// ${(careerData['entry_paths'] as string[]).map((item: string) => `- ${item}`).join('\n')}

// **Possible Specializations:**
// ${(careerData['specializations'] as string[]).map((item: string) => `- ${item}`).join('\n')}
  ...
  try {
    ...
    if (!genAI) {
      ...
      return NextResponse.json({
        role: 'assistant',
        error: 'AI is currently unavailable. Please contact the administrator to set up the Gemini API key.'
      }, { status: 503 });
    }
